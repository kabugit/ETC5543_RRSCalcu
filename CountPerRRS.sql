SELECT h.MasterPatient_ID, GenericIngredientName, COUNT(*) as COUNTER
FROM
(SELECT c.MasterPatient_ID
FROM
(SELECT 
    MasterPatient_ID, 
    SUM(IndRxRisk) AS URxScore, 
    SUM(Weights)   AS WRxScore
FROM
    (   SELECT 
            MasterPatient_ID, 
            ATCComorbCateg, 
            COUNT(DISTINCT ATCComorbCateg) AS IndRxRisk,
            CASE
                WHEN ATCComorbCateg = UPPER('Ischemic Heart Disease: Angina') 
                THEN 2
                WHEN ATCComorbCateg = UPPER('Anticoagulants') 
                THEN 1
                WHEN ATCComorbCateg = UPPER('Antiplatelets') 
                THEN 2
                WHEN ATCComorbCateg = UPPER('Arrhythmia') 
                THEN 2
                WHEN ATCComorbCateg = UPPER('Chronic airways disease') 
                THEN 2
                WHEN ATCComorbCateg = UPPER('Congestive heart failure') 
                THEN 2
                WHEN ATCComorbCateg = UPPER('Hypertension') 
                THEN -1
                WHEN ATCComorbCateg = UPPER('Pulmonary Hypertension') 
                THEN 6
                WHEN ATCComorbCateg = UPPER ('Ischaemic heart disease: Hypertension') 
                THEN -1
                WHEN ATCComorbCateg = UPPER ('Alcohol dependency') 
                THEN 6
                WHEN ATCComorbCateg = UPPER ('Allergies') 
                THEN -1
                WHEN ATCComorbCateg = UPPER ('Anxiety') 
                THEN 2
                WHEN ATCComorbCateg = UPPER ('Benign prostatic hyperplasia') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Bipolar disorder') 
                THEN -1
                WHEN ATCComorbCateg = UPPER ('Dementia') 
                THEN 2
                WHEN ATCComorbCateg = UPPER ('Depression') 
                THEN 2
                WHEN ATCComorbCateg = UPPER ('Diabetes') 
                THEN 2
                WHEN ATCComorbCateg = UPPER ('Epilepsy') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Glaucoma') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Gastrooesophageal reflux disease') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Gout') 
                THEN 1
                WHEN ATCComorbCateg = UPPER ('Hepatitis B') 
                THEN NULL
                WHEN ATCComorbCateg = UPPER ('Hepatitis C') 
                THEN NULL
                WHEN ATCComorbCateg = UPPER ('HIV') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Hyperkalaemia') 
                THEN 4
                WHEN ATCComorbCateg = UPPER ('Hyperlipidaemia') 
                THEN -1
                WHEN ATCComorbCateg = UPPER ('Hyperthyroidism') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Irritable bowel syndrome') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Incontinence') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Inflammation/pain') 
                THEN -1
                WHEN ATCComorbCateg = UPPER ('Liver failure') 
                THEN 3
                WHEN ATCComorbCateg = UPPER ('Malignancies') 
                THEN 2
                WHEN ATCComorbCateg = UPPER ('Malnutrition') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Migraine') 
                THEN -1
                WHEN ATCComorbCateg = UPPER ('Osteoporosis/Paget’s') 
                THEN -1
                WHEN ATCComorbCateg = UPPER ('Pain') 
                THEN 3
                WHEN ATCComorbCateg = UPPER ('Pancreatic insufficiency') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Parkinson’s disease') 
                THEN 3
                WHEN ATCComorbCateg = UPPER ('Psoriasis') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Psychotic illness') 
                THEN 6
                WHEN ATCComorbCateg = UPPER ('Renal disease') 
                THEN 6
                WHEN ATCComorbCateg = UPPER ('Smoking cessation') 
                THEN 6
                WHEN ATCComorbCateg = UPPER ('Steroid-responsive disease') 
                THEN 2
                WHEN ATCComorbCateg = UPPER ('Transplant') 
                THEN 0
                WHEN ATCComorbCateg = UPPER ('Tuberculosis') 
                THEN NULL 
                ELSE 0 
            END AS Weights
        FROM
            (   SELECT 
                ...
                --AND MasterPatient_ID BETWEEN '0' AND '1000000'
                --AND ATCComorbCateg = 'HYPERTENSION' OR ATCComorbCateg = UPPER('Ischaemic heart disease: hypertension')
...
(SELECT 
                ...
                --AND ATCComorbCateg <> '0'
                --AND MasterPatient_ID BETWEEN '0' AND '1000000'
                --AND ATCComorbCateg = 'DIABETES'
                AND GenericIngredientName LIKE '%SACUBIT%'
                --AND ATCComorbCateg = 'HYPERTENSION' OR ATCComorbCateg = UPPER('Ischaemic heart disease: hypertension')                
                GROUP BY 
                    MasterPatient_ID, 
                    ATCComorbCateg, 
                ...
                ORDER BY 
                    ATCLevel5Code) y
ON c.MasterPatient_ID = y.MasterPatient_ID
WHERE URxScore > 13) h
--WHERE URxScore = 1 OR URxScore = 2 OR URxScore = 3 OR URxScore = 4) h
JOIN
(SELECT MasterPatient_ID, GenericIngredientName
FROM DimProductMaster d
JOIN FactScript f ON f.MasterProductId = d.MasterProductID
JOIN DimMAIU m ON m.MAIU_ID = f.MAIUID) g ON h.MasterPatient_ID = g.MasterPatient_ID
GROUP BY h.MasterPatient_ID, GenericIngredientName
