-- Tables with all the combinations
SELECT DISTINCT MasterPatient_ID FROM XX
CROSS JOIN
SELECT DISTINCT ATCComorbCateg FROM XX;

-- 40 conditions for each patient
FULL_table
MAsterPatientID ATCComorbCateg
1234            A
1234            B
...
SELECT  f.*,
        -- If null, it will be replaced the 2nd argument.
        nvl(XX.IndRxRisk, 0)
FROM FULL_TABLE f
LEFT JOIN XX ON f.MasterPatient_ID = XX.MAsterPatient_ID AND f.ATCComorbCateg = XX.ATCComorbCateg
    ...
    (   SELECT 
        ...
        FROM
            (   SELECT 
                ...
                AND MasterPatient_ID BETWEEN 200 AND 1000
                GROUP BY 
                    MasterPatient_ID, 
                    ATCComorbCateg, 
                    ATCLevel5Code
