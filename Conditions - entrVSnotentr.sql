--Entresto patients with therapy initiation date
...
--Cardiac patients overall

CREATE LOCAL TEMP TABLE CardiacPats ON COMMIT PRESERVE ROWS AS
SELECT 
    MasterPatient_ID
FROM
    DimProductMaster d
...
ON
    m.MAIU_ID = f.MAIUID
WHERE 
    ATCLevel1Name ILIKE '%CARDIOVASCULAR SYSTEM%'        
    AND MasterPatient_ID > 0;
    
--Mapping conditions of Entresto patients and taking 2 years look back period
SELECT 
    DISTINCT e.MasterPatient_ID, 
    entrestoInitDate, 
    CASE
        WHEN ( 
        ...
        THEN UPPER('Ischaemic Heart Disease: Angina')
        WHEN (
        ...
        WHEN ATCLevel5Code = 'H03AA01'
        OR  ATCLevel5Code = 'H03AA02'
        THEN UPPER('Hypothyroidism')
        WHEN (
...
WHERE 
    DispenseCalendarDate BETWEEN ADD_MONTHS(entrestoInitDate, -24) AND entrestoInitDate - 1;

--Non-Entresto patients
SELECT 
